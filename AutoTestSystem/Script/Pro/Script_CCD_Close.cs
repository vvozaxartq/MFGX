...
{
    internal class Script_CCD_Close : Script_CCD_Base
    {
        ...
        public override bool Process(CCDBase CCD,ref string strOutData)
        {
            ...
            try
            {
                ...
                if (ret)
                {                
                    outputdata.Add("CCD_Close", "Suceesed");
                    LogMessage($"CCD_Close Suceesed ", MessageLevel.Debug);
                }
                else
                {
                    outputdata.Add("CCD_Close", "Fail");
                    LogMessage($"CCD_Close Fail ", MessageLevel.Debug);
                    pass_fail = false;
                }              
            }
            catch (Exception ex)
            {
                outputdata.Add("CCD_Close", $"Error:{ex.Message}");
