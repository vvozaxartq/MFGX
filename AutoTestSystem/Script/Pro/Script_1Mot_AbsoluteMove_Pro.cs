...
{
    internal class Script_1Mot_AbsoluteMove_Pro : Script_1MotBase
    {
        ...
        public override bool Process(MotionBase MotionDev,ref string output)
        {
            try
            {
                if(MotionDev.Absolute_Move(PLC_command))
                {
            ...
            {
                LogMessage($"Absolute_Move Exception : {e}", MessageLevel.Error);
                return false;
            }
        }
