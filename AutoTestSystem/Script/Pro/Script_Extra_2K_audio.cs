...
namespace AutoTestSystem.Script
{
    ...
    internal class Script_Extra_2K_audio : Script_Extra_Base
    {
        ...
        public double READ_2(string input)
        {
            if(input.Contains("Start: 2000"))
            {
                // 用正則抓 dB 值
                var match = Regex.Match(input, @"Detected peak at \d+(\.\d+)? Hz of (\d+(\.\d+)?) dB");
                if (match.Success)
                {
                    double dbValue = double.Parse(match.Groups[2].Value);
                    return dbValue;
        ...
        public bool READ_CHECK_PARSE(dynamic dutDevice, ref string outputString)
        {
            ...
            LogMessage("\n==============PopQueue Data==============\n" + DataStoreKey);
            double xx = READ_2(DataStoreKey);

            outputString = $"{{ \"value\": {xx} }}";
