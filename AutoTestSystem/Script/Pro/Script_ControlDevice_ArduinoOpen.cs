...
{
    internal class Script_ControlDevice_ArduinoOpen : Script_ControlDevice_Base
    {
        ...
        public override bool Process(ControlDeviceBase ControlDevice, ref string outputjsonStr)
        {
            ...
            try
            {
                ...
                bool ret = ControlDevice.Init("");            
                if (ret)
                {
                    outputdata.Add("ArduinoOpen", "Suceesed");
                    LogMessage($"ArduinoOpen Suceesed ",MessageLevel.Debug);                  
                }
                else
                {
                    outputdata.Add("ArduinoOpen", "Fail");
                    LogMessage($"ArduinoOpen Fail", MessageLevel.Debug);
                    pass_fail =  false;
                }
            }catch(Exception ex)
            {
