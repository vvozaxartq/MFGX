...
{
    internal class Script_DUT_UART_Pro : ScriptDUTBase
    {
        ...
        public override bool Process(DUT_BASE DUTDevice,ref string output)
        {
            ...
            if (TestKeyword == "WaitNormalRespond")
            {
                DUTDevice.READNOJSON(CheckCmd, ref output);
                LogMessage($"Read END:  {output}\n");
            }
            else if (TestKeyword == "Reinit_COM")
            {
                DUTDevice.UnInit();
                if (DUTDevice.Init("") == true)
                {
            ...
            else
            {
                ...
                DUTDevice.READ(CheckCmd, ref output);
                LogMessage($"Read END:  {output}");
            }
