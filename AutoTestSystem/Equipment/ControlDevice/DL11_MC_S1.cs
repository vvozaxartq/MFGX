...
{
    class DL11_MC_S1 : ControlDeviceBase
    {
        private string receivedValue = null;
        [Category("NModBus Parameters"), Description("Select Comport"), TypeConverter(typeof(ComportList))]
        public string PortName { get; set; }

        [Category("NModBus Parameters"), Description("baudRate")]
        public int BaudRate { get; set; } = 9600;

        [Category("NModBus Parameters"), Description("DataBits")]
        ...
        public override bool Init(string strParamInfo)
        {
                ...
                {
                    LogMessage($"can not find ComPort=>{ ex.Message} Init Fail", MessageLevel.Warn);
                    return false;
                }
        ...
        public Dictionary <string,string> ModbusRend(byte slaveAddress, ushort startAddress, ushort numRegisters)
        {
            Dictionary<string, string> strRecAll = new Dictionary<string, string>();
            // 創建Modbus工廠
            var factory = new ModbusFactory();
            // 創建Modbus主機 (Master)
            var modbusMaster = factory.CreateRtuMaster(serialPort);

            ushort[] registers = modbusMaster.ReadHoldingRegisters(slaveAddress, startAddress, numRegisters);

            // 解析數據
            float temperature = registers[0] / 10.0f; // 根據你的設備可能需要調整比例
            float humidity = registers[1] / 10.0f;    // 根據你的設備可能需要調整比例

            LogMessage($"溫度: {temperature}°C 濕度: {humidity}%", MessageLevel.Info);

            strRecAll.Add("Temperature", $"{temperature}");
            strRecAll.Add("Humidity", $"{humidity}");
